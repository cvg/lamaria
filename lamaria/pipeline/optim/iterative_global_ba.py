...
class VIBundleAdjuster:
    ...
    def solve(
        ...
        pyceres_solver_options.minimizer_progress_to_stdout = (
            vi_options.optim.minimizer_progress_to_stdout
        )
        pyceres_solver_options.update_state_every_iteration = (
            vi_options.optim.update_state_every_iteration
        )
        pyceres_solver_options.max_num_iterations = (
            vi_options.optim.max_num_iterations
        )
    ...
    def _init_callback(self, mapper: pycolmap.IncrementalMapper):
        ...
        poses = list(
            mapper.reconstruction.frames[frame_id].rig_from_world
...
class GlobalBundleAdjustment:
    ...
    def adjust(
        ...
        ba_options = self._configure_ba_options(
            pipeline_options, len(reg_image_ids)
        )
...
class IterativeRefinement:
    ...
    def refine(
        ...
        for _ in range(pipeline_options.ba_global_max_refinements):
            ...
            num_changed_observations = mapper.complete_and_merge_tracks(
                tri_options
            )
