...
from ...config.options import TriangulatorOptions, VIOptimizerOptions
...
class VIOptimizer:
    ...
    def process(self, database_path: Path) -> pycolmap.Reconstruction:
        ...
        IterativeRefinement.run(
            self.vi_options, pipeline_options, mapper, self.session
        )
    ...
    def _setup_incremental_mapper(self, database_path: Path):
        ...
        image_names = [
            self.session.data.reconstruction.images[image_id].name
    ...
    def _get_incremental_pipeline_options(self):
        ...
        pipeline_options.triangulation.merge_max_reproj_error = (
            self.triangulator_options.merge_max_reproj_error
        )
        pipeline_options.triangulation.complete_max_reproj_error = (
            self.triangulator_options.complete_max_reproj_error
        )
        pipeline_options.triangulation.min_angle = (
            self.triangulator_options.min_angle
        )

        pipeline_options.mapper.filter_max_reproj_error = (
            self.triangulator_options.filter_max_reproj_error
        )
        pipeline_options.mapper.filter_min_tri_angle = (
            self.triangulator_options.filter_min_tri_angle
        )
